...
// jki
template<typename T>
inline void mm_jki(const T* TACHYON_RESTRICT A,const T* TACHYON_RESTRICT B, T* TACHYON_RESTRICT C, std::size_t M, std::size_t N, std::size_t K) noexcept {
    zero(C, M, N);
    for(size_t j = 0; j < N; ++j) {
        for(size_t k = 0; k < K; ++k) {
            const size_t b_row = k*N;
            for(size_t i = 0; i < M; ++i) {
                C[i*N + j] += A[i*K + k] * B[b_row + j];
            }
        }
    }
}
...
template<typename T>
inline void mm_kji(const T* TACHYON_RESTRICT A,const T* TACHYON_RESTRICT B, T* TACHYON_RESTRICT C, std::size_t M, std::size_t N, std::size_t K) noexcept {
    zero(C, M, N);
    for(size_t k = 0; k < K; ++k) {
        for(size_t j = 0; j < N; ++j) {
        const T b = B[k*N + j];
